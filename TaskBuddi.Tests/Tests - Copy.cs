...
namespace TaskBuddi.Tests
{
    ...
    public class Tests
	{
		...
		[Test]
		public void TaskAndGroupManagement()
		{
			AddGroup();
		...
		public void AddGroup()
		{
			...
			result = app.Query(c => c.Marked("vGroupName").Text("GGG"));
			Assert.IsTrue(result.Any());
			app.Screenshot(TestId + "_GroupAdded");
		}

		public void AddTask()
		{
			// add task			
			app.Tap(c => c.Marked("menu_add_task"));
			app.WaitForElement(c => c.Marked("Task Details"));
			app.EnterText(c => c.Marked("vName"), new string('T', 3));
